 ...
 **生成证书的命令：**
 ...
 `cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=fengjr apiserver-csr.json | cfssljson -bare apiserver`
...
- **6. 创建.kube/config**
  ...
  **生成证书**
  ...
  ``` bash
  cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=fengjr admin-csr.json | cfssljson -bare admin
...
|10.255.72.191|controller03|
  ...
  **生成证书**
  ...
  ```
  cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=fengjr controller-manager-csr.json | cfssljson -bare controller-manager
  ```
...
**九.部署kubelet客户端节点**
  ...
  ``` bash
  cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=fengjr kubelet-csr-189.json | cfssljson -bare kubelet-189
...
``` xml
  ...
  ExecStart=/usr/local/bin/kubelet \
    ...
    --pod-infra-container-image=hub-dev.fengjr.com/k8s/pause-amd64:3.0 \ #公司自建的仓库 搭建参考 harbor[https://github.com/vmware/harbor]
...
``` bash
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=fengjr kube-proxy-csr.json | cfssljson -bare kube-proxy
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: calico-etcd
          image: harbor.fengjr.com/k8s/etcd:2.2.1
          env:
            - name: CALICO_ETCD_IP
              valueFrom:
                fieldRef:
